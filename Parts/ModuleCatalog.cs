...
namespace Solar.Parts
{
    ...
    public static class ModuleCatalog
    {
        ...
        public static List<ModuleDef> BuiltIn() => new()
        {
            ...
            new ModuleDef { Name = "Reaction Wheel", Id = "reaction-wheel", Description = "Gyroscopic attitude control. Provides torque for turning in vacuum without RCS.", Kind = ModuleKind.ReactionWheel, DryMass = 50, Activatable = false, EcDraw = 0.4, Torque = 45000, Tint = new Color(150, 170, 210) },
            new ModuleDef { Name = "Science Jr",     Id = "science-jr",     Description = "Multi-purpose science bay. Runs diverse experiments and returns valuable data.", Kind = ModuleKind.Science,       DryMass = 120, Activatable = true, EcDraw = 0.3, ScienceValue = 12, Tint = new Color(120, 200, 220) },
            ...
            new ModuleDef { Name = "Mystery Goo",   Id = "mystery-goo",  Description = "Exposes a sealed canister of... something... to the space environment. Purely for science!", Kind = ModuleKind.Science, DryMass = 60, Activatable = true, EcDraw = 0.2, ScienceValue = 8, Tint = new Color(150, 210, 120) },
            ...
            new ModuleDef { Name = "Thermometer",   Id = "thermometer",  Description = "Measures ambient temperature. Simple, reliable, and always earns a few science points.", Kind = ModuleKind.Science, DryMass = 40, Activatable = true, EcDraw = 0.2, ScienceValue = 5, Tint = new Color(120, 200, 220) },
            new ModuleDef { Name = "Barometer",     Id = "barometer",    Description = "Measures atmospheric pressure. Works best on bodies with an atmosphere.", Kind = ModuleKind.Science, DryMass = 40, Activatable = true, EcDraw = 0.2, ScienceValue = 5, Tint = new Color(120, 180, 220) },
            ...
            // science instruments
            new ModuleDef { Name = "Gravioli Detector",     Id = "gravioli-detector",     Description = "Detects gravity waves and subtle variations in the local gravitational field.", Kind = ModuleKind.Science, DryMass = 80,  Activatable = true, EcDraw = 0.5, ScienceValue = 18, Tint = new Color(130, 210, 230) },
            new ModuleDef { Name = "Atmosphere Analyzer",   Id = "atmosphere-analyzer",   Description = "Analyzes atmospheric composition in detail. Ideal for probing unfamiliar worlds.", Kind = ModuleKind.Science, DryMass = 65,  Activatable = true, EcDraw = 0.4, ScienceValue = 15, Tint = new Color(130, 190, 225) },
            new ModuleDef { Name = "Seismic Accelerometer", Id = "seismic-accelerometer", Description = "Measures ground vibrations. Deploy on a planet's surface to study its interior.", Kind = ModuleKind.Science, DryMass = 90,  Activatable = true, EcDraw = 0.6, ScienceValue = 16, Tint = new Color(130, 200, 215) },
            new ModuleDef { Name = "Magnetometer",          Id = "magnetometer",          Description = "Maps planetary magnetic fields. Higher returns near magnetospheres.", Kind = ModuleKind.Science, DryMass = 70,  Activatable = true, EcDraw = 0.3, ScienceValue = 14, Tint = new Color(140, 215, 235) },
            new ModuleDef { Name = "Ore Scanner",           Id = "ore-scanner",           Description = "Surveys surface ore concentrations from orbit. Find the best mining spots.", Kind = ModuleKind.Science, DryMass = 60,  Activatable = true, EcDraw = 0.4, ScienceValue = 12, Tint = new Color(145, 205, 225) },
            new ModuleDef { Name = "Docking Sensor",        Id = "docking-sensor",        Description = "Short-range proximity sensor. Helps guide the final meters of a docking approach.", Kind = ModuleKind.Science, DryMass = 25,  Activatable = true, EcDraw = 0.1, ScienceValue = 2, Tint = new Color(200, 185, 210) },
            // power
            ...
            new ModuleDef { Name = "RCS Thruster Block",   Id = "rcs-thruster-block",    Description = "Reaction Control System thruster block. Provides fine translation control.", Kind = ModuleKind.RCS,  DryMass = 50,  Activatable = false, EcDraw = 0.3, RcsThrust = 1000, RcsIsp = 240, Torque = 8000, Tint = new Color(170, 175, 190) },
            // life support
            ...
            // advanced science
            new ModuleDef { Name = "Deep Space Lab",     Id = "deep-space-lab",     Description = "Full pressurized laboratory for long-duration science missions. High-value data returns.", Kind = ModuleKind.Science,      DryMass = 250, Activatable = true, EcDraw = 1.0, SlotCost = 2, ScienceValue = 30, Tint = new Color(110, 190, 225) },
            new ModuleDef { Name = "Radiation Detector", Id = "radiation-detector", Description = "Measures cosmic rays and solar particle flux. Essential data for crewed deep-space missions.", Kind = ModuleKind.Science,      DryMass = 50,  Activatable = true, EcDraw = 0.3, ScienceValue = 10, Tint = new Color(140, 205, 220) },
            new ModuleDef { Name = "Sample Return Capsule", Id = "sample-return-capsule", Description = "Armored container for returning pristine surface samples to Kerbin. Survives re-entry.", Kind = ModuleKind.Science,  DryMass = 40,  Activatable = true, EcDraw = 0.1, ScienceValue = 40, Tint = new Color(180, 160, 120) },
            // advanced comms
            ...
            new ModuleDef { Name = "Large RCS Block",    Id = "large-rcs-block",    Description = "Heavy RCS thruster block for station-keeping and docking of large vessels.", Kind = ModuleKind.RCS,      DryMass = 120, Activatable = false, EcDraw = 0.8, RcsThrust = 2000, RcsIsp = 240, Torque = 16000, Tint = new Color(160, 165, 180) },
            // advanced resource
            ...
            // ---- NEW: colony & spaceflight expansion (curated, existing mechanics; see Progression/TechTree.cs) ----
            // habitation & life support
            new ModuleDef { Name = "Habitation Ring",     Id = "habitation-ring",     Description = "Large rotating habitat ring. Houses a sizeable crew with generous life-support reserves — the heart of a colony.", Kind = ModuleKind.LifeSupport, DryMass = 1400, Activatable = false, CrewCapacity = 6, OxygenCapacity = 648000, WaterCapacity = 360000, FoodCapacity = 240000, EcDraw = 0.6, SlotCost = 2, Tint = new Color(135, 222, 158) },
            new ModuleDef { Name = "Cupola",              Id = "cupola",              Description = "Panoramic observation cupola. Seats a single crew member with a commanding view.", Kind = ModuleKind.LifeSupport, DryMass = 180, Activatable = false, CrewCapacity = 1, OxygenCapacity = 108000, WaterCapacity = 60000, FoodCapacity = 40000, EcDraw = 0.1, Tint = new Color(150, 215, 175) },
            new ModuleDef { Name = "Greenhouse Dome",     Id = "greenhouse-dome",     Description = "Pressurized growing dome. Produces food and oxygen — keeps a surface colony self-sufficient.", Kind = ModuleKind.LifeSupport, DryMass = 420, Activatable = true, FoodCapacity = 60000, OxygenRegen = 0.9, FoodRegen = 0.6, EcDraw = 5.0, SlotCost = 2, Tint = new Color(95, 205, 125) },
            // resource
            new ModuleDef { Name = "Atmospheric Harvester", Id = "atmospheric-harvester", Description = "Scoops and processes atmospheric gases into fuel. Works while landed on a body with an atmosphere.", Kind = ModuleKind.Harvester, DryMass = 320, Activatable = true, FuelProduce = 4.0, EcDraw = 6.0, Tint = new Color(175, 165, 160) },
            // storage
            new ModuleDef { Name = "Snack Container",     Id = "snack-container",     Description = "Sealed food-storage container. Stocks rations for long-duration crews.", Kind = ModuleKind.Storage, DryMass = 90,  FoodCapacity = 120000, Tint = new Color(150, 215, 150) },
            new ModuleDef { Name = "Fuel Depot Pod",      Id = "fuel-depot-pod",      Description = "Bulk fuel depot module. Stores a large monopropellant/fuel reserve for orbital refueling.", Kind = ModuleKind.Storage, DryMass = 220, FuelCapacity = 6000, Tint = new Color(225, 185, 95) },
            // control
            new ModuleDef { Name = "Heavy Reaction Wheel", Id = "heavy-reaction-wheel", Description = "High-torque gyroscopic stabilizer for large stations and motherships.", Kind = ModuleKind.ReactionWheel, DryMass = 180, Activatable = false, EcDraw = 1.2, Torque = 120000, Tint = new Color(145, 165, 205) },
            new ModuleDef { Name = "RCS Quad",            Id = "rcs-quad",            Description = "Four-nozzle RCS cluster. Powerful translation authority for docking heavy vessels.", Kind = ModuleKind.RCS, DryMass = 90, Activatable = false, EcDraw = 0.5, RcsThrust = 4000, RcsIsp = 240, Torque = 20000, Tint = new Color(165, 170, 185) },
            // comms
            new ModuleDef { Name = "Comm-32 Relay",       Id = "comm-32",             Description = "Mid-range relay antenna. Bridges the gap between local antennas and the deep-space network.", Kind = ModuleKind.Antenna, DryMass = 50, Activatable = true, EcDraw = 0.7, Range = 2e11, Relay = true, Tint = new Color(212, 212, 228) },
        };
        ...
        /// <summary>Rewrite a Content directory's modules.json from the in-code <see cref="BuiltIn"/> list.
        /// Used by the <c>--regen-catalogs</c> maintenance command to refresh the source template after the
        /// built-in defaults change — the runtime merge only *adds* new entries, never updates existing
        /// ones, so changed values (e.g. a new ScienceValue) won't reach an existing file otherwise.</summary>
        public static void WriteTemplate(string contentDir)
        {
            var built = BuiltIn();
            var dtos = new List<ModuleDefDto>(built.Count);
            foreach (var m in built) dtos.Add(ModuleDefDto.FromModule(m));
            File.WriteAllText(Path.Combine(contentDir, "modules.json"), JsonSerializer.Serialize(dtos, JsonOpts));
        }
    }
